...
def test_verificar_prestamos_vencidos(biblioteca, usuario1, libro2):
    ...
    for prestamo in biblioteca.libros_prestados:
...
def test_añadir_quitar_libro_disponible(biblioteca, libro1, usuario1):
    biblioteca.ingresar_usuario(usuario1)
    biblioteca.añadir_libro_disponible(libro1)
    assert any(l.nombre == libro1.nombre for l in biblioteca.libros_disponibles)

    biblioteca.quitar_libro_disponible(libro1, usuario1)
    assert not any(l.nombre == libro1.nombre for l in biblioteca.libros_disponibles)
    assert any(l.nombre == libro1.nombre for l in usuario1.libros_prestados)
